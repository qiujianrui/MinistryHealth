package com.library.basemodule.util.camera;
...
import com.library.basemodule.util.camera.listener.ErrorListener;

import com.library.basemodule.util.camera.util.AngleUtil;
import com.library.basemodule.util.camera.util.CameraParamUtil;
import com.library.basemodule.util.camera.util.CheckPermission;
import com.library.basemodule.util.camera.util.DateUtil;
import com.library.basemodule.util.camera.util.DeviceUtil;
import com.library.basemodule.util.camera.util.FileUtil;
import com.library.basemodule.util.camera.util.LogUtil;
import com.library.basemodule.util.camera.util.ScreenUtils;
...
public class CameraInterface implements Camera.PreviewCallback {
    ...
    private boolean isAudio;//标识判断是否获取麦克风 false-为静音视频
    ...
    //视频质量
    private int mediaQuality =  MEDIA_QUALITY_HIGH;
    ...
    private CameraInterface() {
        ...
        SELECTED_CAMERA = CAMERA_FRONT_POSITION; //默认打开前置 前-CAMERA_FRONT_POSITION 后-CAMERA_POST_POSITION
        saveVideoPath = "";
    }
...
//        }
        ...
        videoFileName = DateUtil.date2Str("yyyyMMddHHmmss") + ".mp4";
        if (saveVideoPath.equals("")) {
            saveVideoPath = Environment.getExternalStorageDirectory().getPath()+ File.separator + "MIR";
            File file = new File(saveVideoPath);
            if (!file.exists()) {
                file.mkdirs();
            }
