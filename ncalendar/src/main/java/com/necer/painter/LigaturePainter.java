...
public class LigaturePainter implements CalendarPainter {
    ...
    public LigaturePainter(Context context) {
        ...
        mBgPaint.setColor(Color.parseColor("#3896F3"));
    }
    ...
    private void drawSelectBg(Canvas canvas, RectF rectF, LocalDate localDate, boolean isCurrectMonthOrWeek, List<LocalDate> selectedDateList) {
        ...
        if (selectedDateList.size()>1) {
            if (selectedDateList.get(0).isAfter(selectedDateList.get(1))) {
                endLocalDate = selectedDateList.get(0);
                startLocalDate = selectedDateList.get(1);
            } else {
                endLocalDate = selectedDateList.get(1);
                startLocalDate = selectedDateList.get(0);
            }
        }
        if (selectedDateList.contains(localDate)) {
            if (selectedDateList.size()>1&&startLocalDate!=null&&startLocalDate.equals(localDate)){
                //右矩形 左圆
            ...
            }else if (selectedDateList.size()>1&&endLocalDate!=null&&endLocalDate.equals(localDate)){
                //左矩形 右圆
