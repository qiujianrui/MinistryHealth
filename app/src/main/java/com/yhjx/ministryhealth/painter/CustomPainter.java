...
public class CustomPainter implements CalendarPainter {
    ...
    private List<LocalDate> mBGPointList;
    ...
    private Drawable mBGPointBackground;
    ...
    public CustomPainter(Context context, ICalendar calendar) {
        ...
        mBGPointBackground=ContextCompat.getDrawable(context,R.drawable.n_bg_point);
        ...
        mTodayBackground=ContextCompat.getDrawable(context,R.drawable.n_bg_checked_today);
    ...
    //绘制背景标记
    private void drawBGPoint(Canvas canvas, RectF rectF, LocalDate date, Drawable drawable, int alphaColor) {
        if (mBGPointList.contains(date)) {
            Rect drawableBounds = DrawableUtil.getDrawableBounds((int) rectF.centerX(), (int) rectF.centerY(), drawable);
            drawable.setBounds(drawableBounds);
            drawable.setAlpha(alphaColor);
            drawable.draw(canvas);
    ...
    //设置背景标记
    public void setBGPointList(List<String> list) {
        mBGPointList.clear();
        for (int i = 0; i < list.size(); i++) {
            LocalDate localDate = null;
            try {
            ...
            mBGPointList.add(localDate);
        }
        mCalendar.notifyCalendar();
    }

    public void addBGPointList(List<String> list) {
        for (int i = 0; i < list.size(); i++) {
            LocalDate localDate = null;
            try {
                localDate = new LocalDate(list.get(i));
            ...
            if (!mBGPointList.contains(localDate)) {
                mBGPointList.add(localDate);
            }
        }
        mCalendar.notifyCalendar();
